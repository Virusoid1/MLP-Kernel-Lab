...
- **正确性矩阵**：**226 项 pytest**（207 passed / 0 failed，SwiGLU block 多后端 × shape × dtype + 支持矩阵 + P1 opcheck/gradcheck（swiglu+matmul+layernorm）+ fp16/bf16 训练闭环）；strict FP32 reference 协议；**cuda 算子级 fp16+bf16 全 16 行 PASS（matmul/bias_add/relu/gelu/silu/softmax/swiglu/fused_first）+ 块级 WMMA matmul_half/bf16**
