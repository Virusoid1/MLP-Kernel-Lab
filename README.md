...
- **正确性矩阵**：**213 项 pytest**（181 passed / 0 failed，SwiGLU block 多后端 × shape × dtype + 支持矩阵 + P1 opcheck + fp16/bf16 训练闭环）；strict FP32 reference 协议；cuda 算子级 fp16（swiglu/softmax/relu/gelu/silu）已解锁
