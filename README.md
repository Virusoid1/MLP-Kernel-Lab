...
- **正确性矩阵**：**235 项 pytest**（216 passed / 0 failed，SwiGLU block 多后端 × shape × dtype + 支持矩阵 + P1 opcheck/gradcheck（swiglu+matmul+layernorm）+ fp16/bf16 训练闭环）；strict FP32 reference 协议；**cuda 算子级 fp16+bf16 全 16 行 PASS + conv2d/pool/im2col 三 dtype 齐备 + 块级 WMMA matmul_half/bf16**
