    ...
    int M, int K, int N)
{
    ...
    auto issue = [&](int kt) {
        ...
        // 16B (8-__nv_bfloat16) 向量化 cp.async（v2.12）
...
template<int STAGES>
__global__ __launch_bounds__(128)
void matmul_bf16_pair_swiglu_kernel(
    const __nv_bfloat16* __restrict__ A,
    const __nv_bfloat16* __restrict__ B1, const __nv_bfloat16* __restrict__ B2,
    ...
    int M, int K, int N)
{
    ...
    for (int i = threadIdx.x; i < TILE * TILE; i += blockDim.x) {
        ...
        C[(block_row + r) * N + block_col + c] = __float2bfloat16(gg * sig * uu);
    }
}

void launch_matmul_bf16_pair_swiglu(
    const __nv_bfloat16* A, const __nv_bfloat16* B1, const __nv_bfloat16* B2, __nv_bfloat16* C,
    int M, int K, int N, cudaStream_t stream)
{
    dim3 block(128);
    dim3 grid((N + 31) / 32, (M + 31) / 32);
    matmul_bf16_pair_swiglu_kernel<2><<<grid, block, 0, stream>>>(A, B1, B2, C, M, K, N);
}
