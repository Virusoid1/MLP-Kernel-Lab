...
| dtype 支持矩阵（fp16/bf16） | `tests/test_transformer_mlp.py::DTYPE_SUPPORT` | fp16/bf16 限制实测：cuda=仅FP32(matmul_tiled_auto硬检查)、triton bf16=tl.dot同dtype限制、cutile=ct.mma同dtype限制、eager fp16 scale=1.0时K=3072累加溢出inf；**稳定输入(scale=0.1)下 eager fp16 可用** | 真实边界已量化记录 | E2 |
