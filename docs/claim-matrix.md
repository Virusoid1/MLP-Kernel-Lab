...
| dtype 支持矩阵（fp16/bf16） | `tests/test_transformer_mlp.py::DTYPE_SUPPORT` + **`tests/test_dtype_support_matrix.py`** + `kernels/mlp/matmul_half.cu` | **SwiGLU block fp16：eager/triton/triton_fused/compile/cutile/cuda 全部正确**（norm_l2 2.4e-4~6e-4，3070 实测）。cuda 经 matmul_half（fp16 TensorCore in→fp32 acc→fp16 out, L2 2e-4）解锁；bf16：cuda 仍 fp32-only（binding），其余 5 后端正确 | **四后端 fp16 块级全部可用；cuda fp16 性能 5-7x 慢于 eager16（wmma32 打不过 cuBLAS，诚实记录）** | E2 |
