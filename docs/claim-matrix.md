...
| 测试通过 | `tests/test_*.py` 全量（原算子 55 + SwiGLU + dtype 矩阵 + P1 + 训练闭环 + bf16） | **make reproduce：213 tests（181 passed / 0 failed / 32 skipped，2026-09-02 冻结 93050de）**；各套测试全绿（含 dtype 矩阵 cuda-fp16 5 行转 PASSED + fp16/bf16 训练） | 已复跑通过；**C++ test_kernels 6/6 通过**（2026-09-02 make test-cuda, 3070 sm86, nvcc 13.2：relu/gelu/silu/bias_add/layernorm×2） | E2→E4 |
...
| 算子级 fp16 sweep（2026-09-01, 全 sizes 归档 results/op_bench_20260901_001150.json）+ **2026-09-02 解锁** | Triton elementwise（bias_add_relu 等）fp16 正确且 2048-size 达 1.30x；**CUDA swiglu/softmax/relu/gelu/silu fp16 已解锁**（binding fp16 分派 + half kernels，commit 见下） | **算子级 CUDA fp16 缺口封闭**：dtype matrix cuda-fp16 5 行 PASSED；cuda fp16 block 全自定义（不再回退 F.silu） | E2→E4 |
