...
| 测试通过 | `tests/test_*.py` 全量（原算子 55 + SwiGLU + dtype 矩阵 + P1 + 训练闭环 + bf16） | **make reproduce：226 tests（199 passed / 0 failed / 27 skipped，2026-09-02 冻结 7e09c72）**；各套测试全绿（含 dtype 矩阵 cuda fp16+bf16 10 行转 PASSED + P1 matmul/layernorm opcheck/gradcheck + fp16/bf16 训练） | 已复跑通过；**C++ test_kernels 6/6 通过**（2026-09-02 make test-cuda, 3070 sm86, nvcc 13.2：relu/gelu/silu/bias_add/layernorm×2） | E2→E4 |
...
| dtype 支持矩阵（fp16/bf16） | `tests/test_transformer_mlp.py::DTYPE_SUPPORT` + `tests/test_dtype_support_matrix.py` + `kernels/mlp/matmul_half.cu` | **SwiGLU block fp16 四后端全正确**（norm_l2 2.4e-4~6e-4，3070 实测，42-case sweep correctness 100% 归档）。cuda 经 matmul_half（fp16 in→fp32 acc→fp16 out, L2 2e-4）解锁；bf16：cuda 仍 fp32-only，其余 5 后端正确 | **里程碑：fp16 块级六后端（eager/triton/triton_fused/cuda/cutile/compile）正确性全闭环（3070 + Blackwell 5070 Ti 双架构验证，5070 Ti 上 cuda 为 sm120 单独编译，-k cuda 15p/2s）**；性能：triton 主路径 2.4-3.5x，cuda/cutile 仍慢（诚实记录；Blackwell cuda fp16 详见 artifacts_5070ti/fp16_prefill_cuda_5070ti.json） | E2→E3→E4 |
