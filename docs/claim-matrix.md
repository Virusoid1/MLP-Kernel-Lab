...
| 算子级 fp16/bf16 sweep（2026-09-01 归档 + **2026-09-02 fp16/bf16 双解锁**） | Triton elementwise（bias_add_relu 等）fp16 正确且 2048-size 达 1.30x；**CUDA swiglu/softmax/relu/gelu/silu 的 fp16+bf16 均已解锁**（binding 分派 + half/bf16 kernels） | **算子级 CUDA fp16+bf16 缺口封闭**：dtype matrix cuda 10 行 PASSED（5 fp16 + 5 bf16）；cuda fp16 block 全自定义（不再回退 F.silu） | E2→E4 |
