...
| dtype 支持矩阵（fp16/bf16） | `tests/test_transformer_mlp.py::DTYPE_SUPPORT` + **`tests/test_dtype_support_matrix.py`（执行式探测）** | **算子级 + 块级实测（3070, 2026-09）**：triton 6 算子 + cutile matmul/swiglu 双 dtype 可用（算子级）；**SwiGLU block：eager/triton/triton_fused/compile/cutile fp16+bf16 全部正确**（norm_l2 2.4e-4~4.8e-3；cutile block 经 dtype 传播修复后 6e-4/4.7e-3）；cuda 被 binding float32 硬检查阻塞 | 真实边界固化；**triton fp16 主路径 + cutile fp16 块级可用** | E2 |
