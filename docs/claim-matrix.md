...
| 测试通过 | `tests/test_*.py` 全量（原算子 55 + SwiGLU + dtype 矩阵 + P1 + 训练闭环 + bf16） | **make reproduce：226 tests（203 passed / 0 failed / 23 skipped，2026-09-02 冻结 f43dec8）**；各套测试全绿（dtype 矩阵 cuda 12 行 PASSED + P1 swiglu/matmul/layernorm + fp16/bf16 训练 + C++ test_kernels 6/6） | 已复跑通过 | E2→E4 |
...
| dtype 支持矩阵（fp16/bf16） | `tests/test_transformer_mlp.py::DTYPE_SUPPORT` + `tests/test_dtype_support_matrix.py` + `kernels/mlp/matmul_{half,bf16}.cu` | **SwiGLU block fp16+bf16 三 dtype 全后端正确**：cuda fp16 经 matmul_half、bf16 经 matmul_bf16（均 fp32 acc 输出同 dtype，2026-09-02）；cuda 块级 17p（fp32/fp16/bf16 各 shape 全过，3070+5070 Ti） | **里程碑：fp16+bf16 六后端（eager/triton/triton_fused/cuda/cutile/compile）正确性全闭环，双架构实机验证**；性能：triton 主路径 2.4-3.5x，cuda/cutile 仍慢（诚实记录；Blackwell cuda fp16 详见 artifacts_5070ti/fp16_prefill_cuda_5070ti.json） | E2→E3→E4 |
