...
| 测试通过 | `tests/test_*.py` 全量（原算子 55 + SwiGLU + dtype 矩阵 + P1 + 训练闭环 + bf16） | **make reproduce：235 tests（216 passed / 0 failed / 19 skipped，2026-09-02 冻结 2110876）**；各套测试全绿（**dtype 矩阵 cuda 16/16 行 PASSED**（fp16+bf16 全算子）+ conv/pool 9 测 + P1 swiglu/matmul/layernorm + fp16/bf16 训练 + C++ test_kernels 6/6） | 已复跑通过；Blackwell 全量对齐 216/235 | E2→E4 |
...
| 算子级 fp16/bf16 sweep（2026-09-01 归档 + **2026-09-02 全 dtype 解锁**） | Triton elementwise（bias_add_relu 等）fp16 正确且 2048-size 达 1.30x；**CUDA 全部算子（matmul/bias_add/relu/gelu/silu/softmax/swiglu/fused_first/conv2d/pool/im2col）的 fp16+bf16 均已解锁** | **算子级 CUDA fp16+bf16 缺口彻底封闭**：dtype matrix cuda **16/16 PASS**（8 算子 × 2 dtype）；cuda fp16 block 全自定义；conv/pool 25p 测试 | E2→E4 |
...
| 算子级 fp32 conv/pool 基线（2026-09-01, 归档 results/op_bench_20260901_031557.json） | **conv2d: Triton 0.44x / CUDA 0.71-0.84x vs cuBLAS**（conv 是 cuBLAS 强项，自定义 kernel 慢，如实记录）；conv2d L2 0.0011；softmax 对 conv-shape 不适用（无 M 键，工具边界）；**fp16/bf16 已解锁（2026-09-02：im2col/matmul_half|bf16 组合，cuda_kernels 25p 含 conv/pool 三 dtype）** | E3 算子基线覆盖到 conv/pool；conv 自定义 kernel 慢是真实性能边界；**dtype 完备（性能仍慢，如实记录）** | E3(基线)→E2/E4 |
| 算子级 fp16 全尺寸基线（2026-09-01, 归档 results/op_bench_20260901_040431.json） | matmul/elementwise 全尺寸 fp16：bias_add_relu avg 0.96x（2048-size 1.30x，误差 0）；Triton elementwise fp16 全尺寸正确 | fp16 算子级正确性+性能基线完整归档 | E3(基线) |
| 算子级 bf16 全尺寸基线（2026-09-01, 归档 results/op_bench_20260901_043249.json） | elementwise 全尺寸 bf16：bias_add_relu avg 0.95x（2048-size 1.28x，误差 0）——与 fp16 同档 | **E3 算子基线 fp32/fp16/bf16 三 dtype 补齐** | E3(基线) |
...
| cutile tile 优化（3070, 2026-09 + 09-02）+ **Blackwell 负结果** | `gpu_utils.py` ampere tile (32,32,32)→(32,64,32)→**(64,64,32, 2026-09-02 sweep)**；5070 Ti 84-row sweep | 3070 三档：0.44x→0.55x→**0.55x→1.29x**（M512×4096×11008 内核侧 13.97→9.3ms，1.5x）；**5070 Ti cutile fp16 62.7ms / bf16 59.9ms（负结果，待 Blackwell tile sweep）** | **3070 主 shape 1.3-1.9x 提升 + Blackwell 负结果如实记录** | E3(数据) |
| roofline 效率量化（3070, 2026-09, 从 all-suite 归档） | 计算：6MKF/(median_ms) → TFLOPS vs 3070 理论峰值（fp16-TC≈31 / fp32≈15.9 TFLOPS, mem-BW≈448 GB/s），数据源 artifacts/swiglu_20260901-013853-all-* | **triton fp16 26.7 TFLOPS（86% 峰值）；eager 84%；cutile 36%；cuda wmma32 13%；fp32 51%；decode M=1 带宽 23-36%** | **triton = 3070 fp16 近最优（86%）；decode 带宽-bound 量化确认** | E3 |
| E4 跨设备复现（3080 Ti, 2026-09-01, host c3d5ff0d7eeb） | 同仓库 scp 部署；pytest triton/eager/compile/P1 路径 | **fp16 Triton 加速 2.16x-3.69x vs eager-fp32（6 shapes）；512×4096×11008 2.76ms（1.67x of 3070）；P1 8/8；正确性同档（L2 4.8e-4）** | E4 达成：同架构第二设备复现成功（cutile/cuda 边界见 runbook 5.5） | E4 |
| E4 跨设备复现（3090 Ti, 2026-09-01, host 941ee3922274） | 同仓库 scp 部署；triton/eager/compile/P1 路径 | **fp16 Triton 加速 2.46x-3.40x vs eager-fp32；2048×4096×11008 22.0→7.4ms（2.97x）；decode M1 0.359ms 摊销 86.4x；P1 8/8；正确性 43p** | E4 达成：第二台同架构设备复现 | E4 |
| E4 跨代复现（5070 Ti Blackwell sm120, 2026-09-02, host desktop-hgti0bd） | 同仓库 scp 部署；triton/eager/compile/P1 路径（cutile_probe=True，六后端除 cuda 需编译外全可用） | **fp16 Triton 加速 2.62x-3.00x vs eager-fp32；512×4096×11008 1.52ms；decode M1 0.384ms 摊销 125.2x；P1 8/8；correctness 60 passed** | E4 达成：Blackwell 跨代复现（同精度 vs cuBLAS 0.97-1.07x，无明显优势，如实记录） | E4 |
| E4 跨代复现（5070 Ti Blackwell sm120, 2026-09-02, host desktop-hgti0bd） | 同仓库 scp 部署；triton/eager/compile/P1 路径（cutile_probe=True，六后端除 cuda 需编译外全可用） | **fp16 Triton 加速 2.62x-3.00x vs eager-fp32；512×4096×11008 1.52ms；decode M1 0.384ms 摊销 125.2x；P1 8/8；correctness 60 passed** | E4 达成：Blackwell 跨代复现（同精度 vs cuBLAS 0.97-1.07x，无明显优势，如实记录） | E4 |
...
| PyTorch 自定义算子集成（schema/FakeTensor/opcheck/gradcheck/compile） | `python/torch_registration.py`（torch.library: mlp_kernel::swiglu + **matmul + layernorm**, CPU/CUDA/Meta impl + autograd）+ `tests/test_torch_registration.py` | **21/21 测试通过**（2026-09-02 扩展）：swiglu 8 + matmul 7 + layernorm 6 —— opcheck/gradcheck(fp64)/forward-vs-torch/backward/compile/Meta 全绿 | **已完成并验证（3070），三算子注册闭环** | E2 |
| 多机构建（Ampere/Blackwell lane） | `tools/preflight.py`（lane 判定 + 工具链一致性 + 状态码）+ `docs/compatibility-matrix.md` + `setup.py` build_base 隔离（build/py312-torchX-smCC/） | preflight 在 3070 输出 Ampere lane + TORCH_CUDA_ARCH_LIST=8.6 + status=0；build 隔离验证中 | **已完成（3070 验证）；3080Ti/3090Ti/5070Ti 均已实机跑（lane 识别 + cutile_probe 正确分流）** | E2（本机）/ E4（已完成） |
...
| dtype 支持矩阵（fp16/bf16） | `tests/test_transformer_mlp.py::DTYPE_SUPPORT` + `tests/test_dtype_support_matrix.py` + `kernels/mlp/matmul_{half,bf16}.cu` | **SwiGLU block fp16+bf16 三 dtype 全后端正确**：cuda fp16 经 matmul_half、bf16 经 matmul_bf16（均 fp32 acc 输出同 dtype，2026-09-02）；cuda 块级 17p + **算子级 fp16+bf16 全 16 行 PASS**（含 bias_add/fused_first，2026-09-02 3cebeee） | **里程碑：fp16+bf16 六后端（eager/triton/triton_fused/cuda/cutile/compile）正确性全闭环，双架构实机验证**；性能：triton 主路径 2.4-3.5x，cuda/cutile 仍慢（诚实记录；Blackwell cuda fp16 详见 artifacts_5070ti/fp16_prefill_cuda_5070ti.json） | E2→E3→E4 |
| fp16 TensorCore 性能路径（跨精度 vs 同精度分开：跨精度 FP16/FP32 max 3.37x；同精度 FP16/FP16 仅 1.01-1.17x） | `triton_kernels/matmul.py` + `swiglu_triton.py` 修改（commit 4118882） | **RTX 3070 实测: triton-fp16 vs eager-fp32 = 2.9–3.6x（M≥512, K=4096/F=11008 与 K=768/F=3072 系列）**；fp16 全 sweep 数据存档中 | **v2 第一条性能胜势**；边界：M=128 小 M 时 eager-fp16 反而慢（launch 开销） | E3(部分) |
| 构建架构可移植（非硬编码） | `setup.py` 动态探测（TORCH_CUDA_ARCH_LIST → device capability → 8.6 兜底）；`Makefile` `CUDA_ARCH ?= $(shell ...)`；build 目录按 pyX-torchX-smCC 隔离 | 5070 Ti 干净树（无根目录 .so）经 `setup.py build_ext --inplace` 生成 sm120 产物并 import OK；3070 sm86 同样成立 | **动态架构已落地（v2 改造），Blackwell 干净构建实证** | E2→E4 |
| 不依赖根目录旧 `.so` | 根目录 `mlp_cuda.cpython-312-...so` 为已构建产物（被 `*.so` 忽略） | **已验证（2026-09-02, 5070 Ti）：干净树无 .so → `build_ext --inplace` 生成 sm120 产物并 import OK（torch 2.13+cu130 / nvcc 13.2）**；bench 时 git_dirty=false | **干净构建实证（v2 G1 关闭）** | E4 |
