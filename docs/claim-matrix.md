...
| 测试通过 | `tests/test_*.py` 全量（原算子 55 + SwiGLU + dtype 矩阵 + P1 + 训练闭环 + bf16） | **make reproduce：213 tests（176 passed / 0 failed / 37 skipped，2026-09-01 归档 artifacts/20260901-035836-e562228d-*）**；各套测试全绿（含 dtype 矩阵 20 passed + fp16/bf16 训练） | 已复跑通过；C++ test_kernels 未跑（需手动 nvcc） | E2 |
