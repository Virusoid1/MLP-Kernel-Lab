...
| 训练语义保持（SwiGLU block） | `tests/test_training_loop.py::test_fp16_swiglu_block_training`：X@Wg/X@Wu → mlp_kernel::swiglu（P1 可微 op）→ @Wd 端到端学习 | **fp16 SwiGLU block 收敛：loss 0.104 → 0.038（降 63%）**，梯度全 finite | **v2 核心负载（SwiGLU block）fp16 可端到端训练（3070）** | E2 |
