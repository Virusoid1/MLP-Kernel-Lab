...
| dtype 支持矩阵（fp16/bf16） | `tests/test_transformer_mlp.py::DTYPE_SUPPORT` + **`tests/test_dtype_support_matrix.py`（执行式探测）** | **算子级矩阵实测（3070, 2026-09）**：triton 的 matmul/bias_add/relu/gelu/softmax/swiglu/fused_mlp_first 在 fp16+bf16 **可用**（6×2 组合），triton silu 受 tl.sigmoid 限制；cutile matmul/swiglu 双 dtype **可用**；cuda 全部算子被 binding float32 硬检查阻塞；SwiGLU block fp16/bf16 norm_l2 4.7e-4/3.8e-3 | 真实边界固化；**triton fp16 是主 TensorCore 路径** | E2 |
