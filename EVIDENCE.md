...
|---|---|---|---|
| 全量测试通过 | `make reproduce`（commit 2110876, 2026-09-02 冻结）→ `artifacts/20260902-043239-21108768-*/manifest.json` | **235 tests: 216 passed / 0 failed / 19 skipped**（cuda fp16+bf16 全自定义：算子级全 16 行 PASS + conv/pool 新测 9 + 块级 matmul_half/bf16；P1 swiglu/matmul/layernorm 注册） | E2 |
...
| fp16 六后端性能地图 | `artifacts_3070/swiglu_bench_fp16_bf16.json`（git 冻结 41c7612 终态：cuda cp.async 16B 管线 + pair 融合 + cutile tile64/pair） | M512×4096×11008 fp16：triton 4.46 ≈ eager 5.07 ≈ compile 5.24 < concat 5.70 < triton_fused 6.30 < cutile 6.60（0.77x，原 0.55x/11.4ms）< cuda 13.4ms（0.38x，原 0.16x/31.3ms）；M2048 cuda 0.35x、cutile 0.65x；84 rows 正确性 100% | E3 |
...
| Blackwell 全量复现（2026-09-02, commit 1707aed） | 5070 Ti `tools/reproduce.py --skip-build`（manifest 20260902-045845） | **235 tests: 216 passed / 0 failed / 19 skipped —— 与 3070 冻结基线完全一致**；含 cuTile layernorm Blackwell bug 修复（E[x^2]-mean^2 + 零填充） | E4（全量闭环） |
